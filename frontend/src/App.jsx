...
function TakeExam() {
  ...
  async function check() {
    if (!selected) return
    setBusy(true); setMessage('')
    try { const result = await api.check(id, q.id, selected); setChecked(old => ({ ...old, [q.id]: result })) } catch (e) { setMessage(e.message) } finally { setBusy(false) }
  }
  ...
  return <div className="exam-taking"><div className="exam-top"><Link className="back-link" to={`/exams/${id}/start`}><ArrowLeft size={16} /> Leave session</Link><span className="mode-badge">{mode === 'study' ? 'Study Mode' : 'Exam Mode'}</span></div><div className="exam-heading"><div><p className="eyebrow">{data.course_name.toUpperCase()}</p><h1>{data.title}</h1></div><div className="progress-count">Question <strong>{position + 1}</strong> of {questions.length}</div></div><div className="exam-progress"><span style={{ width: `${((position + 1) / questions.length) * 100}%` }} /></div><div className="exam-workspace"><div className="question-panel"><div className="question-label">QUESTION {position + 1} <span>{answered} of {questions.length} answered</span></div><h2>{q.question_text}</h2><div className="answer-options">{'ABCD'.split('').map(letter => { const state = checked[q.id]; const isSelected = selected === letter; const isCorrect = state?.correct_answer === letter; return <button type="button" disabled={!!state} className={`answer-option ${isSelected ? 'selected' : ''} ${state && isCorrect ? 'correct' : ''} ${state && isSelected && !isCorrect ? 'wrong' : ''}`} key={letter} onClick={() => setAnswers(old => ({ ...old, [q.id]: letter }))}><span className="option-letter">{letter}</span><span>{optionText(q, letter)}</span>{state && isCorrect && <Check size={18} className="answer-check" />}</button> })}</div>{mode === 'study' && checked[q.id] && <div className={`study-feedback ${checked[q.id].is_correct ? 'positive' : 'negative'}`}><strong>{checked[q.id].is_correct ? 'Correct answer!' : 'Not quite.'}</strong> The correct answer is {checked[q.id].correct_answer}: {optionText(q, checked[q.id].correct_answer)}.</div>}{message && <div className="inline-error">{message}</div>}<div className="question-footer"><button className="button secondary" disabled={position === 0} onClick={() => setPosition(p => p - 1)}><ArrowLeft size={16} /> Previous</button><div>{mode === 'study' && !checked[q.id] && <button className="button secondary" disabled={!selected || busy} onClick={check}>{busy ? 'Checking…' : 'Check answer'}</button>}{position < questions.length - 1 ? <button className="button primary" onClick={() => setPosition(p => p + 1)}>Next question <ArrowRight size={16} /></button> : <button className="button primary" disabled={busy} onClick={submit}>{busy ? 'Submitting…' : 'Submit exam'} <ArrowRight size={16} /></button>}</div></div></div><aside className="navigator-panel"><div className="navigator-top"><h3>Questions</h3><span>{answered}/{questions.length} answered</span></div><div className="question-grid">{questions.map((item, index) => <button key={item.id} onClick={() => setPosition(index)} className={`${index === position ? 'current' : ''} ${answers[item.id] ? 'answered' : ''}`} title={`Question ${index + 1}${answers[item.id] ? ' answered' : ' unanswered'}`}>{index + 1}</button>)}</div><div className="legend"><span><i className="legend-current" />Current</span><span><i className="legend-answered" />Answered</span><span><i />Unanswered</span></div><button className="button secondary wide" disabled={busy} onClick={submit}>Submit exam</button></aside></div></div>
}
